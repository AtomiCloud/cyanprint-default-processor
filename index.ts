...
StartProcessorWithLambda(
  async (input, fileHelper): Promise<ProcessorOutput> => {
    ...
    const varEtas: EtaType[] = varEtaConfigs.map(
      (c) => new Eta(c) as any as EtaType,
    );

    const template = fileHelper.resolveAll();
    template
      .map((x) => {
        x.content = varEtas.reduce(
          (acc, eta) => eta.renderString(acc, cfg.vars ?? {}),
          x.content,
        );
        x.relative = varEtas.reduce(
          (acc, eta) => eta.renderString(acc, cfg.vars ?? {}),
          x.relative,
        );
        return x;
